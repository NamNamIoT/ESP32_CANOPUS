...
uint8_t ModbusMaster::readInputRegisters(uint16_t u16ReadAddress,
  uint8_t u16ReadQty)
{
  if(Nelektrik_Serial!=Serial2) {Serial.print("Fail, not kit dev Nelektrik_board"); while(1);}
  _u16ReadAddress = u16ReadAddress;
  _u16ReadQty = u16ReadQty;
...
@return 0 on success; exception number on failure
*/
while(millis()-time_out<=10) {;}//Time out wait for read another register
  /**
@file
